...
import com.king.backend.externaldata.entity.*;
import com.king.backend.externaldata.repository.*;
...
public class PublicDataService {
                            ...
                            if (title != null && !title.isEmpty()) {
                                ...
                                if (content != null) {
                                    ...
                                    if (!existingLink.isPresent()) {
                                        ...
                                        System.out.println("Linked Place ["+place.getName()+"] with Content ["+content.getTitle()+"]");
                                    } else {
                                        System.out.println("Link between Place ["+place.getName()+"] and Content ["+content.getTitle()+"] already exists");
                                    }
                                } else {
                            ...
                            if (nameMedia != null && !nameMedia.isEmpty()) {
                                ...
                                for (Cast cast : matchedCasts) {
                                    ...
                                    if (!existingLink.isPresent()) {
                                        ...
                                        System.out.println("Linked Place ["+place.getName()+"] with Cast ["+cast.getName()+"]");
                                    } else {
                                        System.out.println("Link between Place ["+place.getName()+"] and Cast ["+cast.getName()+"] already exists");
                                    }
                                }
                            } else {
    ...
    private Content searchAndMatchContent(String title) {
        ...
        for (Content content : partialMatches) {
            int similarity = calculateSimilarity(title, content.getTitle());
    ...
    private String getPlaceIdUsingNearbySearch(Place place) {
        try {
            if (place.getLat() == null || place.getLng() == null) return null;
